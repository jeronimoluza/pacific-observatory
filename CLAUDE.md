...
- **Classification** — an **embedding** of the *raw* product name (currently Qwen3-Embedding-4B; previously e5-base → multilingual-e5-large) feeding a **logistic-regression head** that predicts the COICOP leaf. Feed raw product text to the embedder — normalization/canonicalization hurts accuracy. The `canonical_*` keys are **not** classifier inputs.
