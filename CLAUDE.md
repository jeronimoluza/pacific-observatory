...
A Scrapy project (`price_scraping/`) and standalone `fetchers/` are unified under `prices collect`. `enrich/` runs a 3-tier cascade over deduped products: **tier (a)** regex structural extraction (overlays pricing_basis, amount, count, multipack, promos), **tier (b)** KNN over per-country cluster-resolved cache (HNSW indices at `data/prices/_enrich/_tier_b_index/{country}.hnsw`; bootstrap floor `KNN_BOOTSTRAP_CLUSTER_FLOOR = 150`), **tier (c)** KNN-aware LLM reranker for residuals. Tier-(b) cluster_key is `(canonical_strict, country, channel)` — channel is the source-level retailer-type enum (11 values incl. `aggregator`/`hypermarket`/`null`) propagated from per-source YAML through concatenate → prepare. Same-channel K-NN neighbors are preferred; cross-channel falls through when fewer than `MIN_SAME_CHANNEL_KNN=3` same-channel candidates clear the threshold (logged as `cross_channel_accept`). Tier-c narrows the COICOP prompt via `static/channel_coicop_priors.yaml`; out-of-prior outcomes are telemetry-logged to `data/prices/_enrich/_channel_outliers.parquet`. Tier-(b) accepts are guarded by `cluster_agreement_coicop ≥ KNN_CLUSTER_AGREEMENT_MIN` (0.90) plus a pricing_basis-agreement check against tier-a. `backfill/` is the resumable Wayback runner. `build/` (aggregate + basket + fx) and `publish.py` are live at EAP F&B PoC scope — `build` writes `data/prices/_build/eap_fnb_observations.parquet`, `publish` renders `outputs/prices/eap_fnb_dashboard.html` with vendored/inlined Chart.js (WB intranet blocks CDNs).
