...
def test_decide_rows_keys_on_name_and_country():
    """The (name, country) key_fn must route a name to a per-country decision."""
    ...
    out = decide_rows(
        products,
        {key: "01.1.1.1.0", other: "01.1.1.1.0"},
        {key: 0.99, other: 0.10},
        {key: True, other: False},
        {key: 0.97, other: 0.20},
        set(),
        key_fn=lambda p: (str(p["product_name_original"]), str(p["_hlx_country"])),
    )
