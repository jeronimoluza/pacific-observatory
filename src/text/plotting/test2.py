"""test2.py — Option C: host page has zero inline <script>; all dashboard JS
(plus its data) lives in a sibling dashboard.js. Output goes to test2/test2.html
and test2/dashboard.js. Both files must be uploaded into the same directory on
the corporate site so the relative <script src="dashboard.js"> resolves.

The Policy Tracker tab still ships as an iframe srcdoc — that path is already
known to work on the user's corporate intranet."""
...
# ---------- Host HTML (no inline <script>; loads ./dashboard.js) ----------
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .policy-frame { width: 100%; height: 80vh; min-height: 720px; border: 0; border-radius: 8px; background: #f5f6f8; }
        ...
        .toggle-group label { padding: 4px 12px; border: 1px solid #ddd; font-size: 0.82em; font-weight: 400; cursor: pointer; user-select: none; transition: all 0.15s; margin-left: -1px; }
        ...
        .toggle-group label:has(input:checked) { background: #667eea; color: #fff; border-color: #667eea; z-index: 1; position: relative; }
        ...
        .chip-container { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px; max-height: 120px; overflow-y: auto; padding: 4px 0; }
        .chip { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border: 1px solid var(--chip-color, #ddd); border-radius: 16px; font-size: 0.8em; font-weight: 400; cursor: pointer; user-select: none; transition: all 0.15s; }
        .chip:hover { border-color: var(--chip-color, #667eea); background: rgba(102, 126, 234, 0.08); }
        .chip input[type="checkbox"] { display: none; }
        .chip:has(input:checked) { background: var(--chip-color, #667eea); color: #fff; border-color: var(--chip-color, #667eea); }
        .legend-panel { width: 260px; height: 65vh; min-height: 360px; overflow-y: auto; border-left: 1px solid #eee; padding-left: 10px; display: flex; flex-direction: column; gap: 10px; }
        .legend-group { display: flex; flex-direction: column; gap: 4px; border: 1px solid #eee; border-radius: 8px; padding: 6px 8px; cursor: pointer; user-select: none; }
        ...
        .chart-tooltip { position: absolute; background: #fff; border: 1px solid #ddd; border-radius: 6px; box-shadow: 0 6px 18px rgba(0,0,0,0.12); padding: 8px 10px; pointer-events: none; font-size: 0.82em; color: #222; z-index: 10; max-width: 260px; }
        ...
        .noUi-handle { border: 2px solid #667eea !important; border-radius: 50% !important; background: #fff !important; box-shadow: 0 1px 4px rgba(102,126,234,0.35) !important; cursor: ew-resize; }
        ...
        @media (max-width: 900px) {
            ...
            .legend-panel { width: 100%; height: auto; max-height: 160px; border-left: 0; border-top: 1px solid #eee; padding-left: 0; padding-top: 8px; }
        }
    ...
    <div class="shell">
        <div class="header">
            ...
            <div class="tabs" role="tablist">
                <button class="tab is-active" type="button" data-tab="tab-0">Policy Tracker</button>
                <button class="tab" type="button" data-tab="tab-1">Uncertainty Topics</button>
        ...
        <div class="tab-panel is-active" id="tab-0">
            <div class="panel-body">
                <iframe class="policy-frame" srcdoc='__POLICY_TRACKER_SRCDOC__' title="Policy Tracker"></iframe>
            </div>
        </div>
        <div class="tab-panel" id="tab-1">
            <div class="panel-body">
                <div class="controls">
                    ...
                    <select id="topic-country">__TOPIC_OPTIONS__</select>
        ...
        <div class="tab-panel" id="tab-2">
            <div class="panel-body">
                <div class="controls">
                    ...
                    <select id="topics-country">__TOPICS_OPTIONS__</select>
                </div>
                <div class="slider-row">
                    <label>Date Range:</label>
                ...
                <div class="chip-section" id="topics-chip-section">
                    ...
                    <div class="chip-container" id="topics-item-select">__TOPICS_CHIPS__</div>
                </div>
                <div class="plot-row">
                    <div class="chart-wrapper"><canvas id="topics-chart"></canvas></div>
        ...
        <div class="tab-panel" id="tab-3">
            <div class="panel-body">
                <div class="controls">
                    ...
                    <select id="actors-country">__ACTORS_OPTIONS__</select>
                </div>
                <div class="slider-row">
                    <label>Date Range:</label>
                ...
                <div class="chip-section" id="actors-chip-section">
                    ...
                    <div class="chip-container" id="actors-item-select">__ACTORS_CHIPS__</div>
                </div>
                <div class="plot-row">
                    <div class="chart-wrapper"><canvas id="actors-chart"></canvas></div>
...
# ---------- External dashboard.js (data + logic) ----------

DASHBOARD_JS_TEMPLATE = r"""// dashboard.js — bundled data + logic for the Economic/Policy/Uncertainty dashboard.
// Loaded by test2.html via <script src="dashboard.js">. Contains no inline data
// dependencies on the host HTML beyond the IDs of controls and canvases.

const topicData = __TOPIC_DATA_JSON__;
const topicsData = __TOPICS_DATA_JSON__;
const actorsData = __ACTORS_DATA_JSON__;
const topicsItems = __TOPICS_ITEMS_JSON__;
const actorsItems = __ACTORS_ITEMS_JSON__;
const topicsDefaultItems = __TOPICS_DEFAULTS_JSON__;
const actorsDefaultItems = __ACTORS_DEFAULTS_JSON__;

const topicPalette = __PALETTE_JSON__;
const topicsPalette = __PALETTE_JSON__;
const actorsPalette = __PALETTE_JSON__;
...
const tabInit = { 'tab-0': true, 'tab-1': false, 'tab-2': false, 'tab-3': false };
...
function setActiveTab(tabId) {
    ...
    if (window.localStorage) {
        try { window.localStorage.setItem('small-dashboard-tab', tabId); } catch (e) {}
    }
}
...
def build_host_html(
    ...
    policy_srcdoc = escape_srcdoc(_load_policy_html())
    return (
        HOST_TEMPLATE.replace("__POLICY_TRACKER_SRCDOC__", policy_srcdoc)
        .replace("__TOPIC_OPTIONS__", _country_options(topic_data))
        .replace("__TOPICS_OPTIONS__", _country_options(topics_data))
        .replace("__ACTORS_OPTIONS__", _country_options(actors_data))
        .replace("__TOPICS_CHIPS__", _chip_html(topics_items, topics_defaults))
        .replace("__ACTORS_CHIPS__", _chip_html(actors_items, actors_defaults))
    )
...
def build_dashboard_js(
    ...
    return (
        DASHBOARD_JS_TEMPLATE.replace("__TOPIC_DATA_JSON__", json.dumps(topic_data))
        .replace("__TOPICS_DATA_JSON__", json.dumps(topics_data))
        .replace("__ACTORS_DATA_JSON__", json.dumps(actors_data))
        .replace("__TOPICS_ITEMS_JSON__", json.dumps(topics_items))
        .replace("__ACTORS_ITEMS_JSON__", json.dumps(actors_items))
        .replace("__TOPICS_DEFAULTS_JSON__", json.dumps(topics_defaults))
        .replace("__ACTORS_DEFAULTS_JSON__", json.dumps(actors_defaults))
...
def generate_dashboard(
    ...
    html_path = test2_dir / "test2.html"
    js_path = test2_dir / "dashboard.js"
