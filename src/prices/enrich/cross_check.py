"""Cross-check consolidation layer.

Compares the structural extraction's `pricing_basis` against the classified
leaf+sub_label. `consolidate()` returns a routing bucket so the classifier can
arbitrate disagreements via `allowed_bases` instead of the conservative
`pricing_basis_mismatch` refusal.

Buckets:
  PASS_THROUGH    : allowed_bases is permissive — no action
  CLEAN           : structural ∈ allowed — no action
  NO_STRUCTURAL   : structural extraction produced no basis — no action
  SILENT_OVERRIDE : |allowed|=1 and structural disagrees — rewrite basis
  ESCALATE_MULTI  : |allowed|>1 and structural disagrees — leave for review
"""
...
# old amount/unit (whatever structural extraction produced) is no longer
# trustworthy and must be reset together with the basis. count/multiplier are
# basis-orthogonal and are preserved by the caller.
