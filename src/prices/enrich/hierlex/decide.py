...
def _key_fn(p) -> tuple[str, str]:
    return (str(p["product_name_original"]), str(p[_COUNTRY]))


def _bucket_maps(
    shard: pd.DataFrame, tau: float
    ...
    assigned = shard["assigned_coicop"].astype(str).tolist()
    top1 = shard["proposed_leaf"].astype(str).tolist()
    conf = shard["original_score"].astype(float).tolist()
    gate = shard["calibrated_correctness_score"].astype(float).tolist()
    ok = acc & is_leaf
    ...
    return (
        dict(zip(keys, assigned)),
        dict(zip(keys, conf)),
        dict(zip(keys, ok.tolist())),
        dict(zip(keys, gate)),
        dict(zip(keys, top1)),
        fallback_parent,
    )
...
def _decide_bucket(
    ...
    leaf_by, conf_by, ok_by, gate_by, top1_by, fb = _bucket_maps(shard, tau)

    dec = decide_rows(
        group, leaf_by, conf_by, ok_by, gate_by, unembedded, key_fn=_key_fn
    )
    keys = list(zip(group["product_name_original"].astype(str), group[_COUNTRY]))
    # `leaf_top1` documents the head's own argmax, which after a fallback is not
    # the assigned code; restore it from the shard rather than let the assigned
    # action stand in for it.
    dec["leaf_top1"] = [top1_by.get(k) for k in keys]
    if fb:
