...
PROPOSALS_PARQUET = DATA / "base_items_proposals.parquet"
...
def set_data_dir(path) -> None:
    ...
    global DATA, BASE_ITEMS_PARQUET, GAZETTEER_PARQUET, PROPOSALS_PARQUET
...
# §4 shared proposals sink — append-only, keep-first, provenance-stamped. A model
# is only ever a proposer: its verdict is committed here before it seeds anything,
# so the miners replay identically. kind='existence' (Part B) | 'bridge' (Part C).
PROPOSALS_COLS = ["kind", "key", "verdict", "leaf", "provenance", "first_seen", "n"]
...
def head_alias_index() -> dict[str, str]:
    """Derived {alias base_name -> base_item} over every registered alias row.

    The 'main-noun wins' owner lookup: pure function of base_items.parquet, never
    a stored column. When a base_item is a modifier and the syntactic head noun is
    another registered base_item, this index names that head's owner.
    """
    bi = load_base_items()
    al = bi[bi["role"] == "alias"]
    return {str(n).lower(): str(b) for n, b in zip(al["base_name"], al["base_item"])}
...
# leaf-free roles that transfer cleanly across base_items. `form` carries an
# item-specific reroute leaf, so it is deliberately excluded from the global prior.
_PRIOR_ROLES = {"variety", "nonfood", "species_veto"}
...
def build_token_prior(gaz: pd.DataFrame | None = None) -> dict[str, str]:
    """Cross-item (token -> role) prior: a pure projection of gazetteer.parquet.

    A token maps to a transferable role only when every base_item that ever
    labeled it agrees on the same leaf-free role (unanimous) AND at least two
    base_items attest it. No model is ever called — this only re-uses verdicts
    already committed to the gazetteer flywheel.
    """
    if gaz is None:
        gaz = load_gazetteer()
    if gaz.empty:
    ...
    tmp = pd.DataFrame(
        {
            "tok": gaz["token"].astype(str).str.lower(),
            "rb": gaz["role"].astype(str).map(_role_base),
            "item": gaz["base_item"].astype(str),
        }
    )
    prior: dict[str, str] = {}
    for tok, grp in tmp.groupby("tok"):
        roles = set(grp["rb"])
        if len(roles) == 1 and grp["item"].nunique() >= 2:
            (only,) = roles
            if only in _PRIOR_ROLES:
                prior[tok] = only
    return prior


# --- §4 proposals sink (bridge / existence judgment) ---------------------------
def load_proposals() -> pd.DataFrame:
    if PROPOSALS_PARQUET.exists():
        return pd.read_parquet(PROPOSALS_PARQUET)
    return pd.DataFrame(columns=PROPOSALS_COLS)


def append_proposals(rows: list[dict]) -> int:
    """Append proposal rows, keep-first on (kind, key, verdict, leaf). Returns the
    number of NEW rows written. Each row: {kind, key, verdict, leaf, provenance}."""
    cur = load_proposals()
    seen = {(r.kind, r.key, r.verdict, str(r.leaf)) for r in cur.itertuples()}
    new = []
    for r in rows:
        keytup = (r["kind"], r["key"], r["verdict"], str(r.get("leaf") or ""))
        if keytup in seen:
            continue
        seen.add(keytup)
        new.append(
    ...
    if new:
        out = pd.concat([cur, pd.DataFrame(new)], ignore_index=True)
        PROPOSALS_PARQUET.parent.mkdir(parents=True, exist_ok=True)
        out[PROPOSALS_COLS].to_parquet(PROPOSALS_PARQUET, index=False)
    return len(new)
...
def load_record(base_item: str) -> dict:
    ...
    # tokens are whole-alias phrases used for grep + regex-blanking; blank_tokens
    # are their single-word constituents ("olive oil" -> {olive, oil}) used to
    # match/blank individual spaCy tokens, which never carry a multi-word surface.
    blank_tokens = {w for t in tokens for w in t.split()}
    ...
    gaz_all = load_gazetteer()
    gaz = gaz_all[gaz_all["base_item"] == base_item]
    ...
    # global prior (lowest priority): apply unanimous cross-item roles for tokens
    # not already claimed by this item's own aliases/gazetteer, and never for a
    # token that is itself a registered base_item alias (the shared-head guard).
    if not os.environ.get("BASE_ITEMS_DISABLE_PRIOR"):
        owners = head_alias_index()
        claimed = variety | nonfood | species | set(form) | blank_tokens | tokens
        for tok, role in build_token_prior(gaz_all).items():
            if tok in claimed or tok in owners:
                continue
            if role == "variety":
                variety.add(tok)
            elif role == "nonfood":
