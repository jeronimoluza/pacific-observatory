...
- `leaf` — the product clearly belongs to one COICOP consumption leaf. Set `code` to
  that **5-level leaf** (must be a real leaf of COICOP 2018).
...
- **Tea:** ready-to-drink bottled/canned iced tea → `01.2.3.0.3`. Dry herbal/fruit/rooibos/maté infusions, tea substitutes, extracts, instant tea → `01.2.3.0.9`. Dry black/green leaf/bag tea → `01.2.3.0.1`.
...
```json
{
  ...
  "code": "<5-level leaf if verdict=leaf; 4-digit class if verdict=ambiguous_class; empty if exclude>",
...
Rules:
- `code` for a `leaf` verdict MUST be a valid 5-level COICOP 2018 leaf. If you are unsure
  whether a code is a real leaf, step up to `ambiguous_class` with the 4-digit class
