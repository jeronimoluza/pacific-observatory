...
@click.option(
    ...
    "indexes",
    multiple=True,
    help="CC index name like 'CC-MAIN-2024-51'. Repeatable.",
)
@click.option(
    "--workers", "-w", type=int, default=8, show_default=True,
...
def common_crawl_command(
    ...
    if dry_run:
        ...
        if spider_name and spider_name not in SPIDER_CC_CONFIG:
            raise click.BadParameter(
                ...
                f"Available: {sorted(SPIDER_CC_CONFIG)}"
            )
    ...
    if not country:
        raise click.UsageError("--country is required to place output under data/prices/<region>/<sub>/<country>/")
    if not indexes:
        raise click.UsageError(
            "at least one --index is required (e.g. --index CC-MAIN-2024-51)"
        )
