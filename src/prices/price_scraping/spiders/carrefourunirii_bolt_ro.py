"""
Spider for Carrefour Hypermarket Unirii (9166) (Bucharest, Romania) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=137802&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=137802&category_id=<id>&...
...
locale) -- Bucharest coordinates are hardcoded, which is what makes this a
Romania source from any IP.
...
venue-listing endpoint writeup. Measured 29 top-level menu
categories for provider_id=137802 on 2026-09-11.
...
class CarrefourUniriiBoltRoSpider(scrapy.Spider):
    name = "carrefourunirii_bolt_ro"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "RON"
    language = "ro"

    PROVIDER_ID = 137802  # Carrefour Hypermarket Unirii (9166)
    VENUE_URL = "https://food.bolt.eu/en/p/137802-carrefour-hypermarket-unirii-9166/"
    # Bucharest centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Romania.
    LAT = 44.4268
    LON = 26.1025
