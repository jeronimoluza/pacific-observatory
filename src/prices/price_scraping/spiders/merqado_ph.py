"""
Spider for MerQado PH (Philippines) - merqadoph.com.
...
_BASE = "https://www.merqadoph.com"
_API = _BASE + "/products.json"
_PAGE_LIMIT = 250


class MerqadoPhSpider(scrapy.Spider):
    name = "merqado_ph"
    allowed_domains = ["merqadoph.com"]
    currency = "PHP"
    language = "en"
    ...
    def parse_page(self, response):
        ...
        try:
            ...
            logger.error("merqado_ph: non-JSON response p%d", page)
            return
    ...
    def errback(self, failure):
        logger.error(
            "merqado_ph: request failed %s — %r", failure.request.url, failure.value
        )
