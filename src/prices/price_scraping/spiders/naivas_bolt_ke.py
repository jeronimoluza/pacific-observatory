"""
Spider for Naivas Supermarket Ojijo (Nairobi, Kenya) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=186819&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=186819&category_id=<id>&...
...
locale) -- Nairobi coordinates are hardcoded, which is what makes this a
Kenya source from any IP.
...
venue-listing endpoint writeup. Measured 40 top-level menu
categories for provider_id=186819 on 2026-09-11.
...
class NaivasBoltKeSpider(scrapy.Spider):
    name = "naivas_bolt_ke"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "KES"
    language = "en"

    PROVIDER_ID = 186819  # Naivas Supermarket Ojijo
    VENUE_URL = "https://food.bolt.eu/en/p/186819-naivas-supermarket-ojijo/"
    # Nairobi centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Kenya.
    LAT = -1.2921
    LON = 36.8219
