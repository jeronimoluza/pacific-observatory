"""
Spider for Falabella Peru -- https://www.falabella.com.pe/falabella-pe/.

Same Falabella Retail Group Next.js platform and page shape as
falabella_cl.py (Chile): category listing pages embed the full search
result set as JSON in a __NEXT_DATA__ script tag directly under
props.pageProps.results / props.pageProps.pagination, ?page=N advances
pages. Near-verbatim port with the domain, sitemap business-unit code
(PE_FA_COM instead of FA_COM), and currency swapped.

Category discovery uses the site's own sitemap
(static/site/sitemaps/categories/categories_pe_FA_COM-0.xml, 1795 leaf
categories) sampled at a fixed stride, same rationale as falabella_cl.py.

Re-verified live 2026-08-17: GET a Microondas category page (cat40691) ->
200, real PEN prices e.g. "prices":[{"type":"eventPrice","price":["259"]}].
Enumerability proven: page 1 vs page 2 of that category returned 48 and 48
unique productIds respectively with zero overlap.
"""
...
_SITEMAP_URL = "https://www.falabella.com.pe/static/site/sitemaps/categories/categories_pe_FA_COM-0.xml"
_LOC_RE = re.compile(r"<loc>([^<]+)</loc>")
_NEXT_DATA_RE = re.compile(
    r'<script[^>]*id="__NEXT_DATA__"[^>]*>(.*?)</script>', re.DOTALL
)
_CATEGORY_STRIDE = 40  # sample every Nth category from the sitemap (~45 categories)
MAX_PAGES_PER_CATEGORY = 5
_PRICE_PRIORITY = ("internetPrice", "eventPrice", "normalPrice", "cmrPrice")


class FalabellaPeSpider(scrapy.Spider):
    name = "falabella_pe"
