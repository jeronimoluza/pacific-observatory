"""
Spider for Bolt Market Śródmieście (Warsaw, Poland) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=41744&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=41744&category_id=<id>&...
...
locale) -- Warsaw coordinates are hardcoded, which is what makes this a
Poland source from any IP.
...
venue-listing endpoint writeup. Measured 92 top-level menu
categories for provider_id=41744 on 2026-09-11.
...
class BoltmarketBoltPlSpider(scrapy.Spider):
    name = "boltmarket_bolt_pl"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "PLN"
    language = "pl"

    PROVIDER_ID = 41744  # Bolt Market Śródmieście
    VENUE_URL = "https://food.bolt.eu/en/p/41744-bolt-market-srodmiescie/"
    # Warsaw centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Poland.
    LAT = 52.2297
    LON = 21.0122
