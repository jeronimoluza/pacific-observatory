...
_BASE = "https://organica.vn"
_API = _BASE + "/collections/all/products.json"
_PAGE_LIMIT = 50


class OrganicaVnSpider(scrapy.Spider):
    name = "organica_vn"
    allowed_domains = ["organica.vn"]
    currency = "VND"
    language = "vi"
    ...
    def parse_page(self, response):
        ...
        try:
            ...
            logger.error("organica_vn: non-JSON response p%d", page)
            return
    ...
    def errback(self, failure):
        logger.error(
            "organica_vn: request failed %s — %r", failure.request.url, failure.value
        )
