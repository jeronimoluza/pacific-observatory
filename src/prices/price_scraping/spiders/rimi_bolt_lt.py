"""
Spider for Rimi (Mylia) (Vilnius, Lithuania) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=193120&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=193120&category_id=<id>&...
...
locale) -- Vilnius coordinates are hardcoded, which is what makes this a
Lithuania source from any IP.
...
categories for provider_id=193120 on 2026-09-11.
...
class RimiBoltLtSpider(scrapy.Spider):
    name = "rimi_bolt_lt"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "lt"

    PROVIDER_ID = 193120  # Rimi (Mylia)
    VENUE_URL = "https://food.bolt.eu/en/p/193120-rimi-mylia/"
    # Vilnius centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Lithuania.
    LAT = 54.6872
    LON = 25.2797
