"""
Spider for METRO Express Aglantzia (Nicosia, Cyprus) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=194665&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=194665&category_id=<id>&...
...
locale) -- Nicosia coordinates are hardcoded, which is what makes this a
Cyprus source from any IP.
...
venue-listing endpoint writeup. Measured 61 top-level menu
categories for provider_id=194665 on 2026-09-11.
...
class MetroexpressBoltCySpider(scrapy.Spider):
    name = "metroexpress_bolt_cy"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "el"

    PROVIDER_ID = 194665  # METRO Express Aglantzia
    VENUE_URL = "https://food.bolt.eu/en/p/194665-metro-express-aglantzia/"
    # Nicosia centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Cyprus.
    LAT = 35.1856
    LON = 33.3823
