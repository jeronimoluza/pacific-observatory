"""
Spider for Rimi Super (Galerija Centrs) (Riga, Latvia) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=922824&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=922824&category_id=<id>&...
...
locale) -- Riga coordinates are hardcoded, which is what makes this a
Latvia source from any IP.
...
venue-listing endpoint writeup. Measured 31 top-level menu
categories for provider_id=922824 on 2026-09-11.
...
class RimiBoltLvSpider(scrapy.Spider):
    name = "rimi_bolt_lv"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "lv"

    PROVIDER_ID = 922824  # Rimi Super (Galerija Centrs)
    VENUE_URL = "https://food.bolt.eu/en/p/922824-rimi-super-galerija-centrs/"
    # Riga centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Latvia.
    LAT = 56.9496
    LON = 24.1052
