"""
Spider for Supermarket - Karolíny Světlé (Prague, Czech Republic) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=68709&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=68709&category_id=<id>&...
...
locale) -- Prague coordinates are hardcoded, which is what makes this a
Czech Republic source from any IP.
...
venue-listing endpoint writeup. Measured 18 top-level menu
categories for provider_id=68709 on 2026-09-11.
...
class KarolinySvetleBoltCzSpider(scrapy.Spider):
    name = "karolinysvetle_bolt_cz"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "CZK"
    language = "cs"

    PROVIDER_ID = 68709  # Supermarket - Karolíny Světlé
    VENUE_URL = "https://food.bolt.eu/en/p/68709-supermarket-karoliny-svetle/"
    # Prague centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Czech Republic.
    LAT = 50.0755
    LON = 14.4378
