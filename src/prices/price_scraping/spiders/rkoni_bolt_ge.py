"""
Spider for Supermarket Rkoni + (Tbilisi, Georgia) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=27929&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=27929&category_id=<id>&...
...
locale) -- Tbilisi coordinates are hardcoded, which is what makes this a
Georgia source from any IP.
...
venue-listing endpoint writeup. Measured 76 top-level menu
categories for provider_id=27929 on 2026-09-11.
...
class RkoniBoltGeSpider(scrapy.Spider):
    name = "rkoni_bolt_ge"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "GEL"
    language = "ka"

    PROVIDER_ID = 27929  # Supermarket Rkoni +
    VENUE_URL = "https://food.bolt.eu/en/p/27929-supermarket-rkoni/"
    # Tbilisi centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Georgia.
    LAT = 41.7151
    LON = 44.8271
