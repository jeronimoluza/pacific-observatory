"""
Spider for Bravo Ataturk pr (Baku, Azerbaijan) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=157701&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=157701&category_id=<id>&...
...
locale) -- Baku coordinates are hardcoded, which is what makes this a
Azerbaijan source from any IP.
...
venue-listing endpoint writeup. Measured 25 top-level menu
categories for provider_id=157701 on 2026-09-11.
...
class BravoBoltAzSpider(scrapy.Spider):
    name = "bravo_bolt_az"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "AZN"
    language = "az"

    PROVIDER_ID = 157701  # Bravo Ataturk pr
    VENUE_URL = "https://food.bolt.eu/en/p/157701-bravo-ataturk-pr/"
    # Baku centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Azerbaijan.
    LAT = 40.4093
    LON = 49.8671
