"""
Spider for Silver Street Shop 24/7 (Sofia, Bulgaria) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=196940&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=196940&category_id=<id>&...
...
locale) -- Sofia coordinates are hardcoded, which is what makes this a
Bulgaria source from any IP.
...
venue-listing endpoint writeup. Measured 56 top-level menu
categories for provider_id=196940 on 2026-09-11.
...
class SilverstreetBoltBgSpider(scrapy.Spider):
    name = "silverstreet_bolt_bg"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"  # Bulgaria adopted the euro 2026-01-01; the API confirms EUR at collection time
    language = "bg"

    PROVIDER_ID = 196940  # Silver Street Shop 24/7
    VENUE_URL = "https://food.bolt.eu/en/p/196940-silver-street-shop-247/"
    # Sofia centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Bulgaria.
    LAT = 42.6977
    LON = 23.3219
