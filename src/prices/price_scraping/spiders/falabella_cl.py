"""
Spider for Falabella Chile -- https://www.falabella.com/falabella-cl/.

Next.js SSR storefront (homepage is a client shell, but category listing
pages embed the full search result set as JSON in a __NEXT_DATA__ script
tag -- props.pageProps.results / props.pageProps.pagination). ?page=N
query param advances pages.

Category discovery uses the site's own sitemap
(static/site/sitemaps/categories/categories_cl_FA_COM-0.xml, 1707 leaf
categories) sampled at a fixed stride to stay bounded across the whole
catalog rather than crawling all 1707 categories x N pages.

Re-verified live 2026-08-17: GET a Smartphones category page -> 200,
2.5MB, real CLP prices e.g. "prices":[{"type":"normalPrice","price":
["499.990"]}]. Price preference avoids "cmrPrice" (Falabella's own credit
card, not a price a general shopper pays) in favor of internetPrice/
...
_BASE = "https://www.falabella.com"
_SITEMAP_URL = "https://www.falabella.com/static/site/sitemaps/categories/categories_cl_FA_COM-0.xml"
_LOC_RE = re.compile(r"<loc>([^<]+)</loc>")
_NEXT_DATA_RE = re.compile(
    r'<script[^>]*id="__NEXT_DATA__"[^>]*>(.*?)</script>', re.DOTALL
)
_CATEGORY_STRIDE = 40  # sample every Nth category from the sitemap
MAX_PAGES_PER_CATEGORY = 5
_PRICE_PRIORITY = ("internetPrice", "eventPrice", "normalPrice", "cmrPrice")


class FalabellaClSpider(scrapy.Spider):
    name = "falabella_cl"
