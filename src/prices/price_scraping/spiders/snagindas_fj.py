"""S. Nagindas Fiji Odoo apparel storefront."""
...
class SnagindasFjSpider(scrapy.Spider):
    name = "snagindas_fj"
    allowed_domains = ["snagindas.com", "www.snagindas.com"]
    currency = "FJD"
    language = "en"

    start_urls = [
        "https://www.snagindas.com/shop",
    ]
    max_shop_page = 6
    ...
    def _category_from_url(url: str) -> str | None:
        ...
        if not slug:
            return None
        return slug.rsplit("/", 1)[0].rsplit("-", 1)[0].replace("-", " ").title()
