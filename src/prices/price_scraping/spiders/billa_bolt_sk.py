"""
Spider for BILLA Karadžičova (Bratislava, Slovak Republic) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=195121&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=195121&category_id=<id>&...
...
locale) -- Bratislava coordinates are hardcoded, which is what makes this a
Slovak Republic source from any IP.
...
venue-listing endpoint writeup. Measured 30 top-level menu
categories for provider_id=195121 on 2026-09-11.
...
class BillaBoltSkSpider(scrapy.Spider):
    name = "billa_bolt_sk"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "sk"

    PROVIDER_ID = 195121  # BILLA Karadžičova
    VENUE_URL = "https://food.bolt.eu/en/p/195121-billa-karadzicova/"
    # Bratislava centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Slovak Republic.
    LAT = 48.1486
    LON = 17.1077
