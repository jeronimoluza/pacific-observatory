"""
Spider for Selver Torupilli (Tallinn, Estonia) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=160148&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=160148&category_id=<id>&...
...
locale) -- Tallinn coordinates are hardcoded, which is what makes this a
Estonia source from any IP.
...
venue-listing endpoint writeup. Measured 28 top-level menu
categories for provider_id=160148 on 2026-09-11.
...
class SelverBoltEeSpider(scrapy.Spider):
    name = "selver_bolt_ee"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "et"

    PROVIDER_ID = 160148  # Selver Torupilli
    VENUE_URL = "https://food.bolt.eu/en/p/160148-selver-torupilli/"
    # Tallinn centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Estonia.
    LAT = 59.437
    LON = 24.7536
