"""
Spider for Alpro Pharmacy (Malaysia) - alpropharmacy.com.

Shopify storefront; reads the public /products.json feed directly.
Malaysian pharmacy / personal-care catalogue, prices in MYR.
"""
...
_BASE = "https://alpropharmacy.com"
_API = _BASE + "/products.json"
_PAGE_LIMIT = 250


class AlproPharmacySpider(scrapy.Spider):
    name = "alpro_pharmacy"
    allowed_domains = ["alpropharmacy.com"]
    currency = "MYR"
    language = "en"
    ...
    def parse_page(self, response):
        ...
        try:
            ...
            logger.error("alpro_pharmacy: non-JSON response p%d", page)
            return
    ...
    def errback(self, failure):
        logger.error(
            "alpro_pharmacy: request failed %s — %r",
            failure.request.url,
            failure.value,
        )
