"""
Spider for Bolt Market by SPAR - Valletta (Valletta, Malta) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=1132612&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=1132612&category_id=<id>&...
...
locale) -- Valletta coordinates are hardcoded, which is what makes this a
Malta source from any IP.
...
venue-listing endpoint writeup. Measured 26 top-level menu
categories for provider_id=1132612 on 2026-09-11.
...
class BoltmarketBoltMtSpider(scrapy.Spider):
    name = "boltmarket_bolt_mt"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "EUR"
    language = "en"

    PROVIDER_ID = 1132612  # Bolt Market by SPAR - Valletta
    VENUE_URL = "https://food.bolt.eu/en/p/1132612-bolt-market-by-spar-valletta/"
    # Valletta centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Malta.
    LAT = 35.8989
    LON = 14.5146
