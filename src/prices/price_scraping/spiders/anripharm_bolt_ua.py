"""
Spider for ANRI-PHARM (Kyiv, Ukraine) on the Bolt Food delivery
...
surface is the unauthenticated backend it calls:
    ...
    GET https://deliveryuser.live.boltsvc.net/deliveryClient/public/
        getMenuCategories?provider_id=203396&delivery_lat=&delivery_lng=&<client params>
    GET .../getMenuDishes?provider_id=203396&category_id=<id>&...
...
locale) -- Kyiv coordinates are hardcoded, which is what makes this a
Ukraine source from any IP.
...
venue-listing endpoint writeup. Measured 9 top-level menu
categories for provider_id=203396 on 2026-09-11.
...
class AnripharmBoltUaSpider(scrapy.Spider):
    name = "anripharm_bolt_ua"
    allowed_domains = ["deliveryuser.live.boltsvc.net"]
    currency = "UAH"
    language = "uk"

    PROVIDER_ID = 203396  # ANRI-PHARM
    VENUE_URL = "https://food.bolt.eu/en/p/203396-anri-pharm-vul-zolotoustivska-25-zhk-shevchenkivskyi/"
    # Kyiv centre -- the API serves the catalog for whatever country these
    # coordinates fall in, so this is what makes the source Ukraine.
    LAT = 50.4501
    LON = 30.5234
