"""Creative Homes Mongolia home-goods category listings."""
...
_URL = "https://creativehomes.mn/products?category_id=256025"
_COUNTRY = "Mongolia"
_SUBNATIONAL_AREA = "Mongolia online"
_SOURCE_KEY = "mn_creativehomes_mn"
_CURRENCY = "MNT"
_IDENT = ["source_key", "observation_date", "item_name", "source_url"]
_PRICE_RE = re.compile(r"^(?P<name>.+?)\s+(?P<price>[0-9][0-9,]*)\s*₮")
...
def fetch_mn_creativehomes_mn(cutoff: date) -> pd.DataFrame | None:
    obs_date = date.today()
    if obs_date <= cutoff:
    ...
    for anchor in soup.select('a[href^="/products/"]'):
        ...
        row = {
            ...
            "notes": "home goods and decor category product card",
            "scrape_ts": ts,
    ...
    logger.info("[%s] parsed %d home-goods rows", _SOURCE_KEY, len(rows))
