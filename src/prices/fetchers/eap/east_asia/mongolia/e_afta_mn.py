"""E-Afta Mongolia stationery and school-supplies category rows."""
...
_URL = "https://www.e-afta.mn/shop/category/241?order=list_price+desc"
_COUNTRY = "Mongolia"
_SUBNATIONAL_AREA = "Mongolia online"
_SOURCE_KEY = "mn_e_afta_mn"
_CURRENCY = "MNT"
...
def fetch_mn_e_afta_mn(cutoff: date) -> pd.DataFrame | None:
    obs_date = date.today()
    if obs_date <= cutoff:
    ...
    for node in soup.select("div, article, li"):
        ...
        row = {
            ...
            "notes": "stationery/school-supplies category product card",
            "scrape_ts": ts,
    ...
    logger.info("[%s] parsed %d stationery rows", _SOURCE_KEY, len(rows))
