"""WFP food prices (via HDX) — shared MENAAP fetcher, one country per callable.
...
not carry — cereals, pulses, tubers, fresh/dried fish, oils, sugar — across
crisis-affected MENAAP markets. Each country dataset is a single CSV of
per-market monthly observations reaching back years; between these MENAAP
panels this is a general division-01 `official_avg` feed, not a targeted
extractor.

No WFP panel exists on HDX for Morocco or Tunisia (checked via CKAN
package_search on 2026-08-07 -- WFP does not run a price-monitoring
operation there; both are non-operational, middle-income MENAAP countries
for WFP's VAM program). Their `official_avg` coverage has to come from
their own NSOs (HCP / INS) instead.
...
_PANELS: dict[str, tuple[str, str]] = {
    "afg": ("Afghanistan", "wfp-food-prices-for-afghanistan"),
    "lby": ("Libya", "wfp-food-prices-for-libya"),
    "yem": ("Yemen", "wfp-food-prices-for-yemen"),
    "jor": ("Jordan", "wfp-food-prices-for-jordan"),
    "irq": ("Iraq", "wfp-food-prices-for-iraq"),
    "syr": ("Syria", "wfp-food-prices-for-syrian-arab-republic"),
    "pse": ("State of Palestine", "wfp-food-prices-for-state-of-palestine"),
    "irn": (
        "Iran (Islamic Republic of)",
        "wfp-food-prices-for-iran-islamic-republic-of",
    ),
    "lbn": ("Lebanon", "wfp-food-prices-for-lebanon"),
    "dza": ("Algeria", "wfp-food-prices-for-algeria"),
    "dji": ("Djibouti", "wfp-food-prices-for-djibouti"),
    "egy": ("Egypt", "wfp-food-prices-for-egypt"),
}
...
def fetch_wfp_afg(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="afg")


def fetch_wfp_lby(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="lby")


def fetch_wfp_yem(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="yem")


def fetch_wfp_jor(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="jor")


def fetch_wfp_irq(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="irq")


def fetch_wfp_syr(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="syr")


def fetch_wfp_pse(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="pse")


def fetch_wfp_irn(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="irn")


def fetch_wfp_lbn(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="lbn")


def fetch_wfp_dza(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="dza")


def fetch_wfp_dji(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="dji")


def fetch_wfp_egy(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="egy")
