"""WFP food prices (via HDX) — shared LAC fetcher, one country per callable.
...
not carry — cereals, pulses, tubers, fresh produce — across Latin America and
the Caribbean. Each country dataset is a single CSV of per-market monthly
observations reaching back years; between these five LAC panels this is a
general division-01 `official_avg` feed, not a targeted extractor.
...
_PANELS: dict[str, tuple[str, str]] = {
    "col": ("Colombia", "wfp-food-prices-for-colombia"),
    "ecu": ("Ecuador", "wfp-food-prices-for-ecuador"),
    "gtm": ("Guatemala", "wfp-food-prices-for-guatemala"),
    "slv": ("El Salvador", "wfp-food-prices-for-el-salvador"),
    "nic": ("Nicaragua", "wfp-food-prices-for-nicaragua"),
    "bol": (
...
def fetch_wfp_col(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="col")


def fetch_wfp_ecu(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="ecu")


def fetch_wfp_gtm(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="gtm")


def fetch_wfp_slv(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="slv")


def fetch_wfp_nic(cutoff: date) -> pd.DataFrame | None:
    return _fetch(cutoff, iso3="nic")
