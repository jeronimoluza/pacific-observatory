...
SOURCE_META = [
    ...
    },
    {
        ...
        "url": "https://moc.gov.kh/en-US/news/",
        "description": "Official government (Cambodia Ministry of Commerce). Biweekly fuel price notices as news articles (Khmer text, EN/Kh site variants).",
        "extraction_method": ["Web scraping"],
        "products": ["Gasoline (Regular)", "Diesel"],
        "source_keys": ["kh_moc_fuel_notices"],
        "publishes_on": "Biweekly",
        "notes": "Sequential scan of news article IDs from hardcoded ID 3035; checks both /en-US/news/ and /kh/news/ and detects fuel articles via Khmer-script keywords. Price range KHR 2,500–6,500/L.",
    },
]
...
_TMPL_KH_MOC = make_template(
    ...
    source_url="https://moc.gov.kh/en-US/news/",
    currency="KHR",
    unit="L",
...
def fetch_cambodia_moc(cutoff: date) -> pd.DataFrame:
    ...
    for notice_id in range(last_id + 1, last_id + 2000):
        ...
        for lang in news_langs:
            url = f"https://moc.gov.kh/{lang}/news/{notice_id}"
            try:
