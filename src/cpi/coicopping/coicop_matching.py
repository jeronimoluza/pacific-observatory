...
try:
    from .coicop_categories import get_coicop_categories, download_coicop_excel, load_and_process_coicop
...
def setup_google_api_key() -> str:
    ...
    if not api_key:
        ...
        print("   GOOGLE_API_KEY='your-api-key-here' python src/cpi/coicopping/coicop_matching.py")
        print("="*70 + "\n")
...
def download_and_save_coicop_data(project_root: Optional[Path] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    df_no_services[['code', 'title', 'keywords']].to_csv(csv_no_services_path, index=False)
    print(f"✓ Saved {len(df_no_services)} categories (without services) to {csv_no_services_path}")
...
def format_gemini_prompt(batch_products: List[str], coicop_context: pd.DataFrame) -> str:
    """
...
def classify_products_with_gemini(
    ...
    # Prepare data
    unique_products = products_input_df[['product_w_cat']].drop_duplicates().reset_index(drop=True)
    ...
    for batch_num in range(num_batches):
        ...
        batch_products = unique_products.iloc[start_idx:end_idx]['product_w_cat'].tolist()
...
def generate_final_output(
    ...
    final_df = final_df.drop_duplicates(subset=['url_hash', 'product_w_cat'], keep='first')
