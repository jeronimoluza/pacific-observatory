...
def gen_fuel_html(all_data: dict, out: Path):
    ...
    shared_js = r"""
        ...
        // Clean raw product names for display (no data change):
        //  - Strip " average" suffix (GPP artefact)
        //  - Capitalize "(regular)" → "(Regular)" for label consistency
        //  - Capitalize known fuel-family words that appear lowercase mid-string
        //  - Title-case the first character of the result
        ...
        // Suppress (Quality) suffix when quality is already implied by the product name
        ...
        // Build set of product names that appear under >1 fuel_family in rows
        ...
        function chipLabel(key) {
            ...
            var label;
            if (displayProd) {
                // Prefix family only when product name is ambiguous across families
