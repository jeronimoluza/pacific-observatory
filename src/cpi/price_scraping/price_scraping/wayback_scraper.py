...
class WaybackScraper:
    ...
    def _get_existing_url_hashes(self, country: str, stage: str = "items") -> set:
        ...
        """
        wayback_dir = self.output_dir / country / self.spider_name / "wayback_machine_data" / stage
        existing_hashes = set()
    ...
    def _load_snapshots(self, url_hash: str, country: str) -> Optional[List[str]]:
        ...
        """
        snapshots_file = self.output_dir / country / self.spider_name / "wayback_machine_data" / "snapshots" / f"{url_hash}.json"
    ...
    def _save_snapshots(self, url_hash: str, snapshots: List[str], country: str) -> Path:
        """
        ...
        with self._file_write_lock:
            snapshots_dir = self.output_dir / country / self.spider_name / "wayback_machine_data" / "snapshots"
        ...
        try:
            ...
            for row in data[1:]:  # Skip header row
                ...
                if timestamp in seen_timestamps:
                    logger.debug(f"Skipping duplicate snapshot for {url} at {timestamp}")
                    continue
    ...
    def _scrape_wayback_url(self, wayback_url: str) -> Optional[str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(f"HTTP error {e.response.status_code} for {wayback_url} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                return None
            except requests.exceptions.Timeout:
                logger.debug(f"Timeout fetching {wayback_url} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                return None
            except Exception as e:
                logger.debug(f"Failed to fetch {wayback_url} (attempt {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                return None
    ...
    def _snapshot_fetcher_worker(self, input_queue: queue.Queue, output_queue: queue.Queue, country: str, pbar_fetch: tqdm):
        """
        ...
        while True:
            try:
                ...
                if existing_snapshots is not None:
                    ...
                    logger.debug(f"Loaded {len(existing_snapshots)} existing snapshots for {url_hash}")
                ...
                try:
                    ...
                    if snapshots:
                        ...
                        logger.warning(f"No snapshots found for {url} - storing empty list to avoid re-fetching")
    ...
    def _parser_worker(self, input_queue: queue.Queue, country: str, url_hash_to_url: Dict[str, str], pbar_parse: tqdm, stats: Dict[str, Any]):
        """
            ...
            try:
                ...
                for wayback_url in snapshots:
                    try:
                        ...
                        extracted_data = self._extract_data_from_html(html_content, original_url)
                ...
                if failed_count > 0:
                    logger.warning(f"Failed to parse {failed_count}/{len(snapshots)} snapshots for {url_hash} ({original_url})")
    ...
    def _save_parsed_items(self, url_hash: str, data: List[Dict[str, Any]], country: str) -> Path:
        """
        ...
        """
        with self._file_write_lock:
            items_dir = self.output_dir / country / self.spider_name / "wayback_machine_data" / "items"
    ...
    def run_scrape_wayback(self, items: List[Dict[str, Any]], country: str, num_parser_workers: int = 1) -> Dict[str, Any]:
        """
        ...
        items_to_scrape = [(url, url_hash) for url, url_hash in unique_items if url_hash not in existing_items]
        
        logger.info(f"Processing {len(unique_items)} unique URLs from {len(items)} total items")
        logger.info(f"Skipping {len(unique_items) - len(items_to_scrape)} URLs that already have parsed items")
        ...
        logger.info(f"Scraping completed: {stats['successful_scrapes']} successful, {stats['failed_scrapes']} failed, {stats['total_snapshots']} total snapshots")
