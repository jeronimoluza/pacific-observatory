#!/usr/bin/env bash
# Continues the rebuild at MAX_JOBS=50. Excludes sources already completed or
# currently running as orphans from the previous orchestrators.
set -u
export PATH=/usr/local/bin:/usr/bin:/bin:/opt/homebrew/bin:$PATH
cd "$(dirname "$0")/.."

MAX_JOBS="${MAX_JOBS:-50}"
STATUS="logs/rebuild/_status_hiconc.tsv"
mkdir -p logs/rebuild
: > "$STATUS"

# 132 sources: 148 main + 7 empty = 155 total, minus 16 (3 done + 13 running).
# Excluded by name — see /tmp/{done,running}_sources.txt snapshot at launch time.
# Excluded: belapan, charter97, belarusian_partisan (done),
# belta, mlyn, mogilevnews, nasha_niva, neg, novychas, sb, svaboda, vb, zviazda,
# azh_russian, ekonomichna_pravda, sinteza.md (orphaned running).
SOURCES=(
  # ---- Eastern Europe remainder (22) ----
  ...
  # ---- Empty-7 remainder (4): barometr, kyrtag_russian, turkmenistan_gov, aravot ----
  "ca|kyrgyz_republic|barometr" "ca|kyrgyz_republic|kyrtag_russian"
  "ca|turkmenistan|turkmenistan_gov" "so|armenia|aravot"
)
...
total="${#SOURCES[@]}"
echo "Starting hiconc rebuild of $total sources, MAX_JOBS=$MAX_JOBS" | tee -a logs/rebuild/_orchestrator_hiconc.log
idx=0
for entry in "${SOURCES[@]}"; do
  ...
  idx=$((idx+1))
  echo "[$idx/$total] launching $tag/$country/$source" >> logs/rebuild/_orchestrator_hiconc.log
  run_one "$tag" "$country" "$source" &
done

wait
echo "HICONC ALL DONE at $(date -u +%Y-%m-%dT%H:%M:%SZ)" >> logs/rebuild/_orchestrator_hiconc.log
